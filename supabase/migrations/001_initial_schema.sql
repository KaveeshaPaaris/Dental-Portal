-- ============================================================
-- DENTAL CLINIC PLATFORM — Initial Schema Migration
...
-- ─── PROFILES ───────────────────────────────────────────────
-- Extends Supabase auth.users. Only admin staff have accounts.
...
CREATE TABLE bookings (
  ...
  -- Assigned by admin upon acceptance
  assigned_date        DATE,
  assigned_session     session_preference,
  appointment_number   INT,
  slot_order           INT,

  -- OTP — only used if NOT using Twilio Verify API
  -- (can leave nullable; Twilio Verify manages state externally)
  ...
  -- One-time review token (generated when booking is marked COMPLETED)
...
CREATE TABLE inventory (
  ...
  -- Computed: true whenever quantity <= threshold
...
CREATE TABLE inventory_logs (
  ...
  quantity_change INT              NOT NULL, -- positive = added, negative = consumed
...
-- Auto-create notification when stock drops below threshold (on transition only)
...
-- These policies allow authenticated admin users direct Supabase access if ever needed.
-- Public-facing data (accepted reviews, active FAQs, site content) is readable by all.

CREATE POLICY "Public: read accepted reviews"
  ON reviews FOR SELECT USING (status = 'ACCEPTED');
...
-- Service role key bypasses all RLS automatically (used by Express API)

-- ============================================================
-- SEED DATA — Default Site Content Keys
-- ============================================================
...
INSERT INTO faqs (question, answer, keywords) VALUES
  ('What are your clinic hours?',
   'We are open in the morning from 9:00 AM to 1:00 PM, and in the evening from 5:00 PM to 9:00 PM.',
   ARRAY['hours', 'open', 'time', 'schedule', 'when']),

  ('How do I book an appointment?',
   'You can book an appointment online by clicking the "Book an Appointment" button, or by calling/WhatsApp-ing us directly.',
   ARRAY['book', 'appointment', 'schedule', 'reserve']),

  ('Do you accept walk-ins?',
   'We recommend booking in advance, but walk-ins are welcome subject to availability.',
   ARRAY['walk-in', 'walkin', 'without appointment']),

  ('What payment methods do you accept?',
   'We accept cash, credit/debit cards, and most insurance plans. Please contact us for details.',
   ARRAY['payment', 'pay', 'cash', 'card', 'insurance']),

  ('Is there parking available?',
   'Yes, free parking is available for all patients directly outside the clinic.',
   ARRAY['parking', 'park', 'car']),

  ('How do I cancel or reschedule my appointment?',
   'Please call or WhatsApp us at least 24 hours in advance to cancel or reschedule.',
